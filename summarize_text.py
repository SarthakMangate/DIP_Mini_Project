...
# Download 'punkt' for sentence tokenization if not already available
nltk.download('punkt', quiet=True)

# Set Tesseract path for Windows (adjust if necessary)
...
def separate_email_parts(text):
    ...
    # Look for signature markers (from bottom up)
...
def create_summary_image(summary, output_path, mode="general"):
    try:
        ...
        try:
            header_font = ImageFont.truetype("arialbd.ttf", size=26)  # Bold font for header
            body_font = ImageFont.truetype("arial.ttf", size=24)
            signature_font = ImageFont.truetype("ariali.ttf", size=24)  # Italic for signature
        except Exception:
        ...
        if mode.lower() == "email":
            # Separate the summary into header, body, and signature for email formatting
            ...
            if body_text:
                ...
                for line in body_lines:
                    ...
                    y_text += text_height + line_spacing
                y_text += 20
            # Draw a separator for signature if present
            if signature_text:
                draw.line((20, y_text, width - 20, y_text), fill=(200,200,200), width=2)
                y_text += 20
            ...
            # For general text, simply wrap and draw all lines
...
if __name__ == "__main__":
    try:
        ...
        mode = sys.argv[2] if len(sys.argv) > 2 else "general"  # "general" or "email"
